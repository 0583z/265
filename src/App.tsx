...
import { 
  ...
  FileText, Copy, Check, LogOut, Users, Briefcase, LayoutGrid 
} from 'lucide-react';
...
export default function App() {
  ...
  // 🚨 修复 1：强制类型转换，解决缺少 major, techStack 字段的报错
  const allCompetitions = (dbCompetitions.length > 0 ? dbCompetitions : COMPETITIONS) as unknown as Competition[];
  ...
  if (authLoading) return <div className="h-screen bg-zinc-950 flex items-center justify-center text-white italic tracking-widest">GEEK_LOADING...</div>;

  return (
    <div className={(activeTab === 'geek' || activeTab === 'teamup' || activeTab === 'hall') ? 'min-h-screen bg-zinc-950 text-zinc-100' : 'min-h-screen bg-[#FDFDFF] text-gray-900'}>
      <Toaster position="top-center" richColors />
      
      <nav className={`sticky top-0 z-50 backdrop-blur-xl h-20 border-b ${(activeTab === 'geek' || activeTab === 'teamup' || activeTab === 'hall') ? 'border-zinc-800 bg-zinc-950/90' : 'border-gray-100 bg-white/80'}`}>
        <div className="max-w-7xl mx-auto h-full px-6 flex items-center justify-between">
          ...
          <div className="hidden md:flex gap-8 font-bold text-sm uppercase tracking-widest">
            <button onClick={() => setActiveTab('home')} className={`flex items-center gap-2 ${activeTab === 'home' ? 'text-blue-600' : 'hover:text-blue-500 text-gray-400'}`}>探索竞赛</button>
            <button onClick={() => setActiveTab('career')} className={`flex items-center gap-2 ${activeTab === 'career' ? 'text-emerald-500' : 'hover:text-blue-500 text-gray-400'}`}>职业导航</button>
            <button onClick={() => setActiveTab('teamup')} className={`flex items-center gap-2 ${activeTab === 'teamup' ? 'text-fuchsia-500' : 'hover:text-blue-500 text-gray-400'}`}>组队枢纽</button>
            <button onClick={() => setActiveTab('hall')} className={`flex items-center gap-2 ${activeTab === 'hall' ? 'text-yellow-500' : 'hover:text-blue-500 text-gray-400'}`}>精品成果</button>
            <button onClick={() => setActiveTab('profile')} className={`flex items-center gap-2 ${activeTab === 'profile' ? 'text-blue-600' : 'hover:text-blue-500 text-gray-400'}`}>我的档案</button>
            <button onClick={() => setActiveTab('geek')} className={`flex items-center gap-2 ${activeTab === 'geek' ? 'text-emerald-500' : 'hover:text-blue-500 text-gray-400'}`}>极客中心</button>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            {isAuthenticated ? (
              <div className="flex gap-4 items-center">
                <div onClick={() => setActiveTab('profile')} className="px-4 py-1.5 border-2 border-gray-100 rounded-2xl font-black text-xs cursor-pointer hover:bg-gray-50 transition-all active:scale-95">
                  {user?.username}
                </div>
                <LogOut className="w-5 h-5 cursor-pointer text-gray-400 hover:text-red-500 transition-colors" onClick={() => logout()} />
              </div>
            ) : (
              <Button onClick={() => setShowAuthModal(true)} className="bg-blue-600 text-white rounded-xl px-6 font-bold shadow-lg shadow-blue-100">登录 / 注册</Button>
            )}
          </div>
        </div>
      </nav>
        ...
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[95%] max-w-2xl bg-white rounded-[40px] shadow-2xl z-[101] overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-8 overflow-y-auto bg-white">
              <div className="bg-zinc-900 rounded-3xl p-8 relative group border border-zinc-800">
