-- Geek Hub: 组队 + 卷王榜扩展表
-- 执行后可支持 TeamupHub 与 Hall 榜单真实读写链路
...
drop policy if exists "teams leader insert" on public.teams;
create policy "teams leader insert"
on public.teams
for insert to authenticated
with check (auth.uid() = leader_id);

drop policy if exists "teams leader update" on public.teams;
...
drop policy if exists "team applications own insert" on public.team_applications;
create policy "team applications own insert"
on public.team_applications
for insert to authenticated
...
drop policy if exists "team invites inviter insert" on public.team_invites;
create policy "team invites inviter insert"
on public.team_invites
for insert to authenticated
with check (
  auth.uid() = inviter_id
...
drop policy if exists "hall cases public read" on public.hall_of_fame_cases;
create policy "hall cases public read"
